 ...
 */
public class MyAdaptiveGrid<S extends Solution<?>> {
   ...
   */
  public MyAdaptiveGrid(int bisections, int objectives) {
  ...
  public double[] exactLocation(List<S> solutionSet, int index){
    List[] location = new List[numberOfObjectives];
    ...
    return solutionSet.get(index).getObjectives();
  }
  ...
  public String toString() {
    ...
    int n = 5;
    for (int obj = 0; obj < numberOfObjectives; obj++) {
      result += "Objective " + obj + " " + round(gridLowerLimits[obj], n) + " "
              + round(gridUpperLimits[obj], n) + "\n";
    }
    return result;
  }
